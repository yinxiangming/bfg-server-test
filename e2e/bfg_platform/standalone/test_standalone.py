"""
E2E Test 21: Platform Standalone Mode
...
def _get_workspace_base():
    ...
    if not url:
        pytest.skip("WORKSPACE_BASE_URL required for standalone platform e2e")
    return url
...
def platform_api_key():
    ...
    if not key:
        pytest.skip("BFG2_E2E_PLATFORM_API_KEY required for standalone platform e2e")
    return key
...
def admin_token(platform_base):
    ...
    if not email or not password:
        pytest.skip("BFG2_E2E_PLATFORM_EMAIL/PASSWORD required for standalone platform e2e")
...
def new_workspace_slug():
    return f"e2e-standalone-{uuid.uuid4().hex[:8]}"
...
class TestStandaloneTokenExchange:
    ...
    def test_token_exchange_no_membership(self, platform_base, workspace_base):
        ...
        import requests as http
        email = f"e2e-nomember-{uuid.uuid4().hex[:8]}@test.com"
...
class TestStandaloneWorkspaceCreate:
    ...
    def test_create_workspace(self, platform_client, new_workspace_slug):
        ...
        r = platform_client.post(
            ...
            {"name": f"E2E Standalone {new_workspace_slug}", "slug": new_workspace_slug},
        )
...
class TestStandaloneCrossServer:
    ...
    def test_full_create_exchange_access_flow(self, platform_client, workspace_base):
        ...
        slug = f"e2e-full-{uuid.uuid4().hex[:8]}"
