...
|------|-----------|---------------|
| **Embedded** | `api/bfg_platform/embedded/` | Platform runs inside the workspace server (same process, same DB) |
| **Standalone** | `api/bfg_platform/standalone/` | Platform runs as a separate BFG instance with its own DB |
...
In `api/bfg_platform/embedded/.env` (or root `.env`):
```env
...
```bash
BASE_URL=http://localhost:8000 \
  pytest api/bfg_platform/embedded/ -m api_integration -v
```
...
In `api/bfg_platform/standalone/.env` (or root `.env`):
```env
...
```bash
BASE_URL=http://localhost:8011 \
  pytest api/bfg_platform/standalone/ -m api_integration -v
```
